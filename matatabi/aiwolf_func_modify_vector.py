...
class data_info():
    def createDailyVector(self):
        self.daily_vector = np.hstack((self.co_list,self.divined_list.transpose((1,0,2)).reshape(self.agent_num,-1),self.seer_co_oder.reshape(-1,1),self.vote_another_people.reshape(-1,1),self.alive_list,self.ag_esti_list,self.disag_esti_list))
        ...
        if self.each_model == True:
            ...
            if self.daily_train == False:
                for i in range(len(self.daily_net)):
                    serializers.load_npz('./daily_model/agent'+str(self.agent_num)+'/each_model/day_'+str(i)+'/ookawa_train_daily_num_'+str(self.agent_num)+'_day_'+str(i)+'_train_1000.net', self.daily_net[i].net)
            ...
            if self.player_train == False:
                for i in range(len(self.player_net)):
                    serializers.load_npz('./player_model/agent'+str(self.agent_num)+'/each_model/day_'+str(i)+'/ookawa_train_player_num_'+str(self.agent_num)+'_day_'+str(i)+'_train_1000.net', self.player_net[i].net)
        else:
            self.daily_net = [predict_werewolf(n_input=self.daily_vector_length, n_hidden=200, n_output=self.agent_num)]
            if self.daily_train == False:
                serializers.load_npz("./daily_model/agent"+str(self.agent_num)+"/one_model/ookawa_train_daily_num_"+str(self.agent_num)+"_train_1000.net", self.daily_net[0].net)
            self.player_net = [predict_role(n_input=self.player_vector_length,n_hidden=50,n_output=self.role_num)]
            if self.player_train == False:
                serializers.load_npz("./player_model/agent"+str(self.agent_num)+"/one_model/ookawa_train_player_num_"+str(self.agent_num)+"_train_1000.net", self.player_net[0].net)
