...
class SampleAgent(object):
    def __init__(self, agent_name):
        ...
        self.player_vector_length = 4*self.agent_num + self.role_num + 5 
        ...
        self.daily_net = [MLP(n_input=self.agent_num*(self.player_vector_length)+1,n_hidden=200,n_output=self.agent_num) for i in range(self.agent_num+1)]
        self.player_net = [MLP(n_input=self.player_vector_length+1,n_hidden=50,n_output=self.role_num) for i in range(self.agent_num+1)]


        # serializers.load_npz("/Users/matatabinoneko/University/AIWOLF/ookawa_replay/daily_model/ookawa_train_daily_"+str(self.agent_num)+".net", self.daily_net[0])
        # serializers.load_npz("/Users/matatabinoneko/University/AIWOLF/ookawa_replay/player_model/ookawa_train_player_" + str(self.agent_num) + ".net", self.player_net[0])
        # serializers.load_npz("/Users/matatabinoneko/University/AIWOLF/ookawa_replay/daily_model/ookawa_daily.net", self.daily_net[0])
        # serializers.load_npz("/Users/matatabinoneko/University/AIWOLF/ookawa_replay/player_model/ookawa_player.net", self.player_net[0])
        # serializers.load_npz("/Users/matatabinoneko/University/AIWOLF/ookawa_replay/daily_model/ookawa_train_only_daily.net", self.daily_net[0])
        # serializers.load_npz("/Users/matatabinoneko/University/AIWOLF/ookawa_replay/player_model/ookawa_train_only_player.net", self.player_net[0])
        
        if self.daily_train == False:
            for i in range(len(self.daily_net)):
                serializers.load_npz('./daily_model/agent'+str(self.agent_num)+'/each_model/day_'+str(i)+'/ookawa_train_daily_num_'+str(self.agent_num)+'_day_'+str(i)+'_train_10000.net', self.daily_net[i])
        if self.player_train == False:
            for i in range(len(self.player_net)):
                serializers.load_npz('./player_model/agent'+str(self.agent_num)+'/each_model/day_'+str(i)+'/ookawa_train_player_num_'+str(self.agent_num)+'_day_'+str(i)+'_train_10000.net', self.player_net[i])

        self.daily_optimizer = [chainer.optimizers.Adam() for i in range(len(self.daily_net))]
        for i in range(len(self.daily_optimizer)):
            self.daily_optimizer[i].setup(self.daily_net[i])

        self.player_optimizer = [chainer.optimizers.Adam() for i in range(len(self.player_net))]
        for i in range(len(self.player_optimizer)):
            self.player_optimizer[i].setup(self.player_net[i])

        self.train_cnt = 1

        if self.daily_train == True:
            self.daily_x_t = [[] for i in range(len(self.daily_net))]
        if self.player_train == True:
            self.player_x_t = [[]for i in range(len(self.player_net))]

        self.daily_loss_sum = [[]for i in range(len(self.daily_net))]
        self.daily_accu_sum = [[]for i in range(len(self.daily_net))]
        self.daily_loss_mean = [[]for i in range(len(self.daily_net))]
        self.daily_accu_mean = [[]for i in range(len(self.daily_net))]
        self.player_loss_sum = [[]for i in range(len(self.player_net))]
        self.player_accu_sum = [[]for i in range(len(self.player_net))]
        self.player_loss_mean = [[]for i in range(len(self.player_net))]
        self.player_accu_mean = [[]for i in range(len(self.player_net))]
    ...
    def randomSelect(self):
        while(True):
            ...
            if target != self.base_info["agentIdx"]-1 and self.alive_list[target][0]==1:
                return target

    def selectAgent(self,target_role):
        # if target_role == 'WEREWOLF':
        # print(self.co_list.shape,self.divined_list.reshape(self.agent_num,-1).shape,self.seer_co_oder.reshape(-1,1).shape,self.vote_another_people.reshape(-1,1).shape,self.alive_list.shape,self.ag_esti_list.shape,self.disag_esti_list.shape)
        self.daily_vector = np.hstack((self.co_list,self.divined_list.reshape(self.agent_num,-1),self.seer_co_oder.reshape(-1,1),self.vote_another_people.reshape(-1,1),self.alive_list,self.ag_esti_list,self.disag_esti_list))
        est_werewolf_list = self.daily_net[self.base_info["day"]](np.asarray([self.base_info["day"],] + self.daily_vector.reshape(-1).tolist()).astype(np.float32).reshape(1,-1)).reshape(-1).array
        est_werewolf_list = list(zip(est_werewolf_list,range(1,len(est_werewolf_list)+1)))
        est_role_list = []
        for i in range(self.agent_num):
            role = np.argmax(self.player_net[self.base_info["day"]](np.asarray([self.base_info["day"],] + self.daily_vector[i,:].reshape(-1).tolist()).astype(np.float32).reshape(1,-1)).reshape(-1).array)
        ...
        if target_role == "WEREWOLF":
            ...
            for _,target in est_werewolf_list:
                if self.alive_list[target-1][0] == 1 and target != self.base_info['agentIdx']:
                    return target
    ...
    def updateAliveList(self,index):
        if self.diff_data["type"][index] == "dead":
            ...
            self.alive_list[agent][2] = 1
        elif self.diff_data["type"][index] == "execute":
            agent = self.diff_data["agent"][len(self.diff_data)-1]-1
            self.alive_list[agent][1] = 1
    ...
    def updateVector(self):
        ...
        # self.daily_vector = np.hstack((divined_human_werewolf_list,self.seer_co_oder.reshape(-1,1),divine_human_werewolf_list,self.vote_another_people.reshape(-1,1),self.alive_list,self.ag_esti_list,self.disag_esti_list))
        self.daily_vector = np.hstack((self.co_list,self.divined_list.reshape(self.agent_num,-1),self.seer_co_oder.reshape(-1,1),self.vote_another_people.reshape(-1,1),self.alive_list,self.ag_esti_list,self.disag_esti_list))
        # if base_info["day"] == 2:
    ...
    def initialize(self, base_info, diff_data, game_setting):
        ...
        ### edit from here ###
        # self.daily_vector = np.zeros((self.agent_num*(5+self.role_num+3*self.agent_num)),dtype=np.int32)
        ...
        self.alive_list = np.zeros((self.agent_num,3),dtype=np.int32)#alive，attacked，voted
        self.alive_list[:,0] = 1
    ...
    def finish(self):
        ...
        if self.daily_train == True:
            for i in range(1,len(self.daily_x)):
                self.daily_x_t[i].append(self.daily_x[i] + self.daily_t)
                    # print(len(self.daily_t))

        if self.player_train == True:
            for i in range(len(self.player_x)):
                for j in range(len(self.player_x[i])):
                    self.player_x_t[i].append(self.player_x[i][j]+[self.player_t[j]])
        ...
        for i in range(len(self.daily_net)):
            ...
            if self.daily_train == True:
                if 10 < len(self.daily_x_t[i]):
                    random_index = np.random.choice(len(self.daily_x_t[i]),10)
                    # print((np.asarray(self.daily_x_t)[:,self.player_vector_length]).reshape)
                    # print((np.asarray(self.daily_x_t)[:,-1])[:,1])
                    daily_x_100 = np.asarray(self.daily_x_t[i])[:,:-1*self.agent_num][random_index,:].astype(np.float32)
                    daily_t_100 = np.array(self.daily_x_t[i])[:,-1*self.agent_num:][random_index].astype(np.int32)

                    daily_y_100 = self.daily_net[i](daily_x_100)
                    ...
                    self.daily_net[i].cleargrads()
                    loss.backward()
                    self.daily_optimizer[i].update()
                ...
                with chainer.using_config("train", False), chainer.using_config("enable_backprop", False):
                    for i in range(1,len(self.daily_x)):
                        ...
                        daily_y = self.daily_net[i](daily_x)
                        ...
                        for j in range(len(daily_y)):
                            if daily_y[0][j] == 1 and self.base_info["statusMap"].get(str(j+1)) == 'Alive':
                                self.using_alive_info_cnt_daily+=1

        for i in range(len(self.player_net)):
            if self.player_train == True:
                if 10 < len(self.player_x_t[i]):
                    player_x_t_100 = np.asarray(self.player_x_t[i])[np.random.choice(len(self.player_x_t[i]),10),:]
                    player_x_100 = player_x_t_100[:,:-1].astype(np.float32)
                    player_t_100 = player_x_t_100[:,-1].astype(np.int32)

                    player_y_100 = self.player_net[i](player_x_100)
                    loss = F.softmax_cross_entropy(player_y_100,player_t_100)
                    accuracy = F.accuracy(player_y_100,player_t_100)        
                    self.player_net[i].cleargrads()
                    loss.backward()
                    self.player_optimizer[i].update()

                    self.player_loss_sum[i].append(loss.array)
                    self.player_accu_sum[i].append(accuracy.array)
                ...
                with chainer.using_config("train", False), chainer.using_config("enable_backprop", False):
                    for i in range(1,len(self.player_x)):
                        if 0 < len(self.player_x[i]):
                            ...
                            player_y = self.player_net[i](player_x)
                            ...
                            for j in range(len(player_y)):
                                if self.num_to_role[np.argmax(player_y[j])] in self.werewolf_list:
                                    if self.alive_list[j][1] != 1:
        ...
        if(self.train_cnt == self.train_times):
            if self.daily_train == False:
                ...
                # for i in range(len(self.daily_net)):
                #     if len(self.daily_x_t[i]) == 0:
                #         continue
                #     daily_x = np.asarray(self.daily_x_t[i])[:,:-1*self.agent_num].astype(np.float32)
                #     daily_t = np.array(self.daily_x_t[i])[:,-1*self.agent_num:].astype(np.int32)
                #     batch_size = 10

                #     with chainer.using_config("train", False), chainer.using_config("enable_backprop", False):
                #         for j in range(0,len(daily_x),batch_size):
                #             daily_y = self.daily_net[i](daily_x[j:j+batch_size])
                ...
                #             self.daily_loss_sum[i].append(loss.array)
                #             self.daily_accu_sum[i].append(accu)
                #             self.daily_loss_mean[i].append(np.mean(self.daily_loss_sum[i]))
                #             self.daily_accu_mean[i].append(np.mean(self.daily_accu_sum[i]))
            else:
                for i in range(len(self.daily_net)):
                    chainer.serializers.save_npz('./net_folder/ookawa_train_daily_num_'+str(self.agent_num)+'_day_'+str(i)+'.net', self.daily_net[i])
            ...
            if self.player_train == False:
                ...
                # for k in range(len(self.player_net)):
                #     if len(self.player_x_t[k])==0:
                #         continue
                #     player_x_t = np.asarray(self.player_x_t[k])
                ...
                #             player_y = self.player_net[k](player_x[i:i+batch_size])
                #             loss = F.softmax_cross_entropy(player_y,player_t[i:i+batch_size])
                #             accu = F.accuracy(player_y,player_t[i:i+batch_size])

                #             self.player_loss_sum[k].append(loss.array)
                #             self.player_accu_sum[k].append(accu.array)
                #             self.player_loss_mean[k].append(np.mean(self.player_loss_sum[k]))
                #             self.player_accu_mean[k].append(np.mean(self.player_accu_sum[k]))
                ...
                for i in range(len(self.player_net)):
                    chainer.serializers.save_npz('./net_folder/ookawa_train_player_num_'+str(self.agent_num)+'_day_'+str(i)+'.net', self.player_net[i])
        ...
        #     print(len(self.daily_x_t[i]),len(self.player_x_t[i]))
        if self.train_cnt%1000 == 0:
            for i in range(len(self.daily_net)):
                ...
                chainer.serializers.save_npz(daily_path+file_name, self.daily_net[i])
            for i in range(len(self.player_net)):
                ...
                chainer.serializers.save_npz(player_path+file_name, self.player_net[i])
