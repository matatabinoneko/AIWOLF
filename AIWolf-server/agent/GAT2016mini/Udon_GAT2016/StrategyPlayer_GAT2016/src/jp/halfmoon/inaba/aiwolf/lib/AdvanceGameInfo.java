...
//		long starttime = System.currentTimeMillis();
		...
		switch( gameInfo.getRole() ){
			case WEREWOLF:
				// 狼に既に騙りがいれば潜伏、いなければ60%で占騙り。騙り役職で初期化し、騙りをやめるときは村人で上書きする
				if( Math.random() < 0.6 ){
					fakeRole = Role.SEER;
				}else{
...
//		System.out.println("AdvanceGameInfo InitTime:" + (endtime - starttime));
		...
		for (int i = readTalkListNum; i < talkList.size(); i++) {
			...
			switch (utterance.getTopic())
			{
				...
				case GUARDED:
					...
					// CO情報の更新
					if( updateCommingOut( guardedAgentNo, Role.BODYGUARD, talk ) ){
						// CO撤回発生フラグを立てる
						existCancel = true;
					}
	...
	private boolean updateCommingOut(int agentNo, Role role, Talk commingOutTalk){
		...
		// TODO このクラスは視点整理用なので騙り役職の制御を行うのは不適切。外で制御する
		// 狼の騙り役職制御（他の人狼が占を騙ったら自分は村を騙る）
		if( latestGameInfo.getRole() == Role.WEREWOLF ){
			// 自分が未COか（CO済なら無理に変えない）
			if( agentState[latestGameInfo.getAgent().getAgentIdx()].comingOutRole == null ){
				for( Integer wolfAgentNo : getWolfList() ){
					if( wolfAgentNo == agentNo && wolfAgentNo != latestGameInfo.getAgent().getAgentIdx() ){
						fakeRole = Role.VILLAGER;
					}
				}
			}
