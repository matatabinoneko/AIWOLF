...
class MyData():
    ...
    def addBGData(self, agentIdx, targetIdx, day, turn):
        if (agentIdx not in self.__bgCODataMap_):
            ...
            if (agentIdx in self.__mediumCODataMap_):
                data.setAfterSlide(True)
                self.__mediumCODataMap_[agentIdx].setAlive(False)
            if (agentIdx in self.seerCODataMap_):
                data.setAfterSlide(True)
                self.seerCODataMap_[agentIdx].setAlive(False)
        else:
    ...
    def getMaxLikelyWolfAll(self):
        #candidate = self.getAliveAgentList()
        #perm = np.random.permutation(self.AgentToIndexList(candidate))
        #return perm[0]
        ...
        maxRate = float("-inf")
        maxIdxList = {}
        for i in enmuerate(self.__aliveAgentList_)
            wolfRate = wolfSideTable_.getWolfRate(idx)
            if (maxRate < wolfRate):
                maxRate = wolfRate
				maxIdxList.clear()
				maxIdxList.append(idx)
            elif (maxRate == wolfRate):
                maxIdxList.append(idx)
        if (maxIdxList.isEmpty()):
            return -1
        elif (maxIdxList.size() == 1) {
			return maxIdxList.get(0);
		else {
			return maxIdxList.get(r_.nextInt(maxIdxList.size()));
        '''
    ...
    def getMaxLikelyWolf(self,targetList):
		...
		ArrayList<Integer> maxIdxList = new ArrayList<Integer>();
		for (int idx : targetIdxList) {
			if (isAlive(idx)) {
				double wolfRate = wolfSideTable_.getWolfRate(idx);
				if (maxRate < wolfRate) {
					maxRate = wolfRate;
					maxIdxList.clear();
					maxIdxList.append(idx);
				} else if (maxRate == wolfRate) {
					maxIdxList.append(idx);
				}
			}
		}
		if (maxIdxList.isEmpty()) {
			return targetIdxList.get(0);
		} else if (maxIdxList.size() == 1) {
			return maxIdxList.get(0);
		} else {
			return maxIdxList.get(r_.nextInt(maxIdxList.size()));
		}
    ...
    def getMinLikelyWolfAll(self):
		...
		minRate = float("-inf")
		minIdxList = []
		for idx enumerate(self.__aliveAgentList_):
            nWolfRate = self.wolfSideTable_.getWolfRate(idx)
			if (minRate > nWolfRate):
				minRate = nWolfRate
				minIdxList.clear()
				minIdxList.append(idx)
            else(minRate == nWolfRate):
                minIdxList.append(idx)

		if (minIdxList.isEmpty()):    return -1
        elif (len(minIdxList) == 1):    return minIdxList.get(0)
        return minIdxList.get(r_.nextInt(minIdxList.size()))
        '''

    def getMinLikelyWolf(self,targetList):
        perm = np.random.permutation(targetList)
        return perm[0]
    ...
    def getLikelyWolfRate(self):
        '''
        wolfRates = []
        for idx in self.getAliveAgentList():
            wolfRates.append(wolfSideTable_.getWolfRate(idx))
        return wolfRates
        '''
    ...
    def getLikelyWolfRateMap(self):
        ...
        wolfRates = []
        for idx in self.getAliveAgentList():
            wolfRates.update(idx, wolfSideTable_.getWolfRate(agentIdx))
		return wolfRates
        '''
    ...
    def getMaxLikelyVillAll(self):
		...
		double maxRate = Double.NEGATIVE_INFINITY;
		ArrayList<Integer> maxIdxList = new ArrayList<Integer>();
		for (int idx : getAliveAgentList()) {
			double nWolfRate = wolfSideTable_.getNotWolfRate(idx);
			if (maxRate < nWolfRate) {
				maxRate = nWolfRate;
				maxIdxList.clear();
				maxIdxList.append(idx);
			} else if (maxRate == nWolfRate) {
				maxIdxList.append(idx);
			}
		}
		if (maxIdxList.isEmpty()) {
    ...
    '''
	public ArrayList<Double> getLikelyVillRate() {
		ArrayList<Double> nWolfRates = new ArrayList<Double>();
		for (int agentIdx : getAliveAgentList()) {
			nWolfRates.append(wolfSideTable_.getNotWolfRate(agentIdx));
		}

		return nWolfRates;
	}

	public HashMap<Integer, Double> getLikelyVillRateMap() {
		HashMap<Integer, Double> nWolfRates = new HashMap<Integer, Double>();
		for (int agentIdx : getAliveAgentList()) {
			nWolfRates.update(agentIdx, wolfSideTable_.getNotWolfRate(agentIdx));
		}
