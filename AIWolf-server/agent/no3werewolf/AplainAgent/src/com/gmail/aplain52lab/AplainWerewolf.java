...
public class AplainWerewolf extends AbstractWerewolf {
	...
	List<Agent> possesseds; //裏切り者である可能性がほかよりも高い者のリスト
	...
	public void update(GameInfo gameInfo) {
		...
		// 明らかな裏切り者候補がいない時、同一のCOをしている人たちを裏切り者だと疑う
		if (possessedPersons.isEmpty()) {
			possessedPersons.addAll(possesseds);
		}
	...
	public String talk() {
		...
		//もし占い師や霊媒師が人間側から２人以上でたら片方は偽物確定（狂人候補）
		int seerCO = 0;
		int mediumCO =0;
		...
		for (Agent agent : humans) {
			...
			if (agi.getComingoutMap().get(agent) == Role.SEER) {
				seerCO++;
				seerList.add(agent);
			} else if (agi.getComingoutMap().get(agent) == Role.MEDIUM) {
				mediumCO++;
				mediumList.add(agent);
			}
		}
		if(seerCO >= 2) {
			fakeSeerCO++;
			for(Agent agent : seerList) {
				if(!possesseds.contains(agent)) {
					possesseds.add(agent);
				}
			}
		}
		if(mediumCO >= 2) {
	...
	void chooseVoteCandidate() {
		...
		//投票意見が半数を超えていれば、人狼陣営でもそちらに投票する（ライン切り）
		for (Agent agent : agi.getAliveOthers()) {
			if(agi.getVoteCounter().get(agent).intValue() >= agi.getAliveOthers().size()/2) {
				candidates.clear();
				voteCandidate = agent;
				return;
			}
