 ...
 */
public class AplainPossessed extends AbstractPossessed {
	...
	void chooseVoteCandidate() {
		...
		if (fakeRole == Role.SEER || fakeRole == Role.MEDIUM) {
			//投票意見が半数を超えていれば、人狼候補であろうとそちらに投票する（ライン切り）
	...
	Judge getFakeJudge(Role role) {
		...
		if (AplainWerewolf.countWolfJudge(judgeList) < gameSetting.getRoleNum(Role.WEREWOLF) && Math.random() < 0.3) {
