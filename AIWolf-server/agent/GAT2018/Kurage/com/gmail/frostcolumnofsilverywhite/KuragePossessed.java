 ...
 * 裏切り者役エージェントクラス
 */
public class KuragePossessed extends KurageVillager {
    int numWolves;
    boolean isCameout;
    ...
    private Judge getFakeDivination() {
        ...
        for (Agent agent : aliveOthers) {
            if (!divinedAgents.contains(agent) && database.getRole(agent) != Role.SEER) {
    ...
    protected void chooseVoteCandidate() {
        ...
        for (Agent agent : aliveOthers) {
            if (!werewolves.contains(agent) && database.getRole(agent) == Role.SEER) {
                candidates.add(agent);
            }
        }
        // 人狼と判定したエージェントは投票先候補
        ...
        for (Judge j : fakeDivinationList) {
            if (j.getResult() == Species.HUMAN) {
                ...
                if (!candidates.contains(j.getTarget()) && aliveOthers.contains(j.getTarget())) {
