 ...
 * 人狼役エージェントクラス
 */
public class KurageWerewolf extends KurageBasePlayer {
    /** 既定人狼数 */
    int numWolves;
    /** 騙る役職 */
    Role fakeRole;
    ...
    public void update(GameInfo gameInfo) {
        ...
        for (Judge j : divinationList) {
            ...
            if (!werewolves.contains(agent) && 
                ((humans.contains(j.getTarget()) && j.getResult() == Species.WEREWOLF) || 
    ...
    private Judge getFakeJudge() {
        ...
        if (fakeRole == Role.SEER) {
            ...
            for (Agent agent : aliveOthers) {
                if (!fakeJudgeMap.containsKey(agent) && database.getRole(agent) != Role.SEER) {
        ...
        if (target != null) {
            ...
            if (humans.contains(target)) {
                ...
                if (nFakeWolves < numWolves) {
                    ...
                    if (possessedList.contains(target) || !database.isCo(target)) {
                        if (Math.random() < 0.5) {
    ...
    protected void chooseVoteCandidate() {
        ...
        if (fakeRole != Role.VILLAGER) {
            ...
            for (Agent agent : villagers) {
                if (database.getRole(agent) == fakeRole || fakeJudgeMap.get(agent) == Species.WEREWOLF) {
                    candidates.add(agent);
                }
            }
        ...
        if (fakeRole != Role.VILLAGER) {
            if (!isCameout) {
                ...
                for (Agent agent : werewolves) {
                    if (database.getRole(agent) == Role.SEER) {
                        fakeSeerCo++;
                    }
                    else if (database.getRole(agent) == Role.MEDIUM) {
                        fakeMediumCo++;
                    }
                }
                if (fakeRole == Role.SEER && fakeSeerCo > 0 || 
                    ...
                    for (Agent agent : humans) {
                        if (database.getRole(agent) == fakeRole) {
                            comingoutDay = day;
                        }
                    }
