 ...
 * 霊媒師役エージェントクラス
 */
public class KurageMedium extends KurageVillager {
    int comingoutDay;
    boolean isCameout;
    ...
    protected void chooseVoteCandidate() {
        ...
        for (Agent agent : aliveOthers) {
            if (database.getRole(agent) == Role.MEDIUM) {
            ...
            if (!werewolves.contains(voteCandidate)) {
                ...
                if (canTalk) {
                    talkQueue.offer(new Content(
                            new EstimateContentBuilder(voteCandidate, Role.WEREWOLF)));
                    talkQueue.offer(new Content(
                            new RequestContentBuilder(null, new Content(
                                    new DivinationContentBuilder(voteCandidate)))));
                }
            }
        }
    }
