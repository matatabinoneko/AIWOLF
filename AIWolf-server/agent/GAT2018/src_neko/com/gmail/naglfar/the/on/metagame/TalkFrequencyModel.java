...
import com.gmail.naglfar.the.on.framework.GameEventListenr;
...
public class TalkFrequencyModel implements GameEventListenr, MetagameEventListener {
    ...
    private Map<Role, Double> calcRoleProbability(GameAgent agent, Topic topic, int day, int turn, Map<Role, Double> originalP) {
        ...
        for (Entry<Role, Double> entry : originalP.entrySet()) {
            ...
            newP.put(role, getTopicConditionalProbability(agent, day, turn, role, topic));
        }
        return newP;
    }
    ...
    private double getTopicConditionalProbability(GameAgent agent, int day, int turn, Role role, Topic topic) {
        ...
        hh = h.HMM1(agent,day,turn,role,topic);
        //System.out.println("hh:"+hh );
        tcp.get(day).get(turn).put(role, new HashMap<>());
        tcp.get(day).get(turn).get(role).put(topic, hh);
    ...
    private Map<Topic, Double> getCommonTCP(int day, int turn, Role role) {
        ...
        if (P == null) {
            ...
            try (BufferedReader br = new BufferedReader(new InputStreamReader(TalkFrequencyModel.class.getClassLoader().getResourceAsStream("com/gmail/naglfar/the/on/metagame/" + filename)))) {;
                String line = null;
