...
class Environment():
    def __init__(self,agent_num=5,train_predict_mode=False,train_dqn_mode=False,train_divine_mode=False,train_times=1000,predict_net_load=False,dqn_net_load=False,divine_net_load=False,test_train_mode=False,each_model=False,epsilon=0.3,kanning=False,explore=False,pred_h=100,dqn_h=100):
        # super(self).__init__()
        ...
        self.black_divine = 0
        self.win_ratio_list = []
        self.r_pred_cnt = defaultdict(int)
        self.r_correct_pred_cnt = defaultdict(int)
        ...
        self.player = Agent(pred_n_input=self.daily_vector_length, pred_n_hidden=self.pred_h, pred_n_output=self.agent_num*self.role_num,dqn_n_input=self.daily_vector_length+self.agent_num*self.role_num,dqn_n_hidden=self.dqn_h,dqn_n_output=self.agent_num, agent_num=self.agent_num,role_num=self.role_num,t_role_cnt = self.t_role_cnt,train_predict_mode=self.train_predict_mode,train_dqn_mode=self.train_dqn_mode,train_divine_mode=self.train_divine_mode,explore=explore)
        # self.player = Agent(pred_n_input=self.daily_vector_length, pred_n_hidden=self.pred_h, pred_n_output=self.agent_num*self.role_num,dqn_n_input=self.daily_vector_length,dqn_n_hidden=self.dqn_h,dqn_n_output=self.agent_num, agent_num=self.agent_num,role_num=self.role_num,t_role_cnt = self.t_role_cnt,train_predict_mode=self.train_predict_mode,train_dqn_mode=self.train_dqn_mode,train_divine_mode=self.train_divine_mode,explore=explore)
        # self.player = Agent(pred_n_input=self.daily_vector_length, pred_n_hidden=self.pred_h, pred_n_output=self.agent_num*self.role_num,dqn_n_input=self.agent_num*self.role_num,dqn_n_hidden=self.dqn_h,dqn_n_output=self.agent_num, agent_num=self.agent_num,role_num=self.role_num,t_role_cnt = self.t_role_cnt,train_predict_mode=self.train_predict_mode,train_dqn_mode=self.train_dqn_mode,train_divine_mode=self.train_divine_mode,explore=explore)
        ...
        if self.predict_net_load == True:
            if self.each_model == True:
                ...
                self.player.pred_model.model = torch.load('./net_folder/predict_model/agent'+str(self.agent_num)+ '/' + str(self.pred_h) + '/modify_predict_role_train_daily_num_'+str(self.agent_num)+'_train_10000.net')
                print("predict model is loaded.")
        if self.dqn_net_load == True:
            path = './net_folder/dqn_model/agent'+str(self.agent_num)+'/'+str(self.dqn_h)+'/'
            file_name = 'dqn_num_'+str(self.agent_num)+'_train.net'
            print("dqn model is loaded.")
            self.player.brain.main_q_model = torch.load(path+file_name)
        if self.divine_net_load == True:
            path = './net_folder/divine_model/agent'+str(self.agent_num)+'/'+str(self.dqn_h)+'/'
            file_name = 'divine_num_'+str(self.agent_num)+'_train.net'
    ...
    def update_predict_result(self,pred_y,t):
        tmp_y = np.array(np.argsort(np.argsort(-1*pred_y,axis=2),axis=2)<1)
        y_list = [self.decode(y) for y in tmp_y]
        tmp_t = self.decode(np.array(t).reshape(self.agent_num,self.role_num))

        for y in y_list:
            for y_role, t_role in zip(y,tmp_t):
            ...
            # print()
            if y[self.base_info["agentIdx"]-1] == tmp_t[self.base_info["agentIdx"]-1]:
        ...
        tmp_y = pred_y.transpose(0,2,1)
        # print(tmp_y)
        for i in range(tmp_y.shape[0]):
            for j in range(tmp_y.shape[1]):
                tmp_y[i][j] = np.array(np.argsort(np.argsort(-tmp_y[i][j],axis=0),axis=0)<self.utiwake[self.num_to_role[j]])
        # for i in range(tmp_y.shape[0]):
        #     for j in range(tmp_y.shape[1]):
        #         for k in range(tmp_y.shape[2]):
        #             tmp_y[i][j][k] = 1 if tmp[i][j][k] 

        tmp_t = np.array(t).reshape(self.agent_num,self.role_num).transpose()

        # print(tmp_y,tmp_t)
        y_list = [[np.where(y==True) for y in y_] for y_ in tmp_y]
        for i,y_ in enumerate(y_list):
            for j,y in enumerate(y_):#役職数
                for k in y[0].tolist():#プレイヤ数
                    #tmp[i][j][k]=1である
                    self.r_pred_cnt[self.num_to_role[j]] += 1
                    if tmp_t[j][k] == 1:
                        # print()
                        self.r_correct_pred_cnt[self.num_to_role[j]] += 1
    ...
    def save_one_model(self):
        daily_path = './net_folder/predict_model/agent'+str(self.agent_num) + '/' + str(self.pred_h) + '/'
        os.makedirs(daily_path,exist_ok=True)
    ...
    def display_game_result(self):
        ...
        for role in self.role_to_num.keys():
            print("role to player")
            print("{:<10}, {:>10}times, {:>10}predicts".format(role , self.role_cnt[role],self.r_pred_cnt[role]))
            if self.role_cnt[role] != 0 and self.r_pred_cnt[role] != 0:
                recall=self.r_correct_pred_cnt[role]/self.role_cnt[role]
                precision=self.r_correct_pred_cnt[role]/self.predict_cnt[role]
                f_1 = 2*recall*precision/(recall+precision)   
                print("{:<10}, recall:{:<.3f}, precision:{:<.3f}, f-1:{:<.3f}\n".format(role,recall, precision,f_1))
    ...
    def finish(self):
        ...
        # self.writer.add_scalar('data/win_ratio',win,len(self.win_ratio_list))
        ...
        if(self.train_cnt % self.train_times == 0):
