...
class SampleAgent(object):
    def __init__(self, agent_name):
        ...
        self.daily_net = [MLP(n_input=(self.player_vector_length-2)*self.agent_num+2,n_hidden=200,n_output=self.agent_num) for i in range(self.agent_num+1)]
        self.player_net = [MLP(n_input=self.player_vector_length,n_hidden=50,n_output=self.role_num) for i in range(self.agent_num+1)]
        ...
        if self.daily_train == False:
            for i in range(len(self.daily_net)):
                serializers.load_npz('./daily_model/agent'+str(self.agent_num)+'/each_model/day_'+str(i)+'/ookawa_train_daily_num_'+str(self.agent_num)+'_day_'+str(i)+'_train_4000.net', self.daily_net[i])
        if self.player_train == False:
            for i in range(len(self.player_net)):
                serializers.load_npz('./player_model/agent'+str(self.agent_num)+'/each_model/day_'+str(i)+'/ookawa_train_player_num_'+str(self.agent_num)+'_day_'+str(i)+'_train_4000.net', self.player_net[i])
    ...
    def selectAgent(self,target_role):
        ...
        est_werewolf_list = self.daily_net[self.base_info["day"]](np.asarray([self.base_info["day"],np.sum(self.co_list,axis=0)[1]] + self.daily_vector.reshape(-1).tolist()).astype(np.float32).reshape(1,-1)).reshape(-1).array
        est_werewolf_list = list(zip(est_werewolf_list,range(1,len(est_werewolf_list)+1)))
        est_role_list = []
        for i in range(self.agent_num):
            role = np.argmax(self.player_net[self.base_info["day"]](np.asarray([self.base_info["day"],np.sum(self.co_list,axis=0)[1]] + self.daily_vector[i,:].reshape(-1).tolist()).astype(np.float32).reshape(1,-1)).reshape(-1).array)
