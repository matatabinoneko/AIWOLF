...
class data_info():
    def __init__(self,agent_num=5,daily_train=False,player_train=False,train_times=1000,each_model=True):
        ...
        # self.daily_loss_mean = [[]for i in range(self.agent_num)]
        # self.daily_accu_mean = [[]for i in range(self.agent_num)]
        # self.player_loss_mean = [[]for i in range(self.agent_num)]
        # self.player_accu_mean = [[]for i in range(self.agent_num)]

        if self.each_model == True:
            self.daily_net = [predict_werewolf(n_input=(self.player_vector_length-2)*self.agent_num+2, n_hidden=200, n_output=self.agent_num) for i in range(self.agent_num)]
            if self.daily_train == False:
                for i in range(len(self.daily_net)):
                    serializers.load_npz('./daily_model/agent'+str(self.agent_num)+'/each_model/day_'+str(i)+'/ookawa_train_daily_num_'+str(self.agent_num)+'_day_'+str(i)+'_train_100.net', self.daily_net[i].net)
            ...
            if self.player_train == False:
                for i in range(len(self.player_net)):
                    serializers.load_npz('./player_model/agent'+str(self.agent_num)+'/each_model/day_'+str(i)+'/ookawa_train_player_num_'+str(self.agent_num)+'_day_'+str(i)+'_train_100.net', self.player_net[i].net)
        else:
            self.daily_net = [predict_werewolf(n_input=(self.player_vector_length-2)*self.agent_num+2, n_hidden=200, n_output=self.agent_num)]
            if self.daily_train == False:
                serializers.load_npz("./daily_model/agent"+str(self.agent_num)+"/one_model/ookawa_train_daily_num_"+str(self.agent_num)+"_train_8000.net", self.daily_net[0].net)
            self.player_net = [predict_role(n_input=self.player_vector_length,n_hidden=50,n_output=self.role_num)]
            if self.player_train == False:
                serializers.load_npz("./player_model/agent"+str(self.agent_num)+"/one_model/ookawa_train_player_num_"+str(self.agent_num)+"_train_8000.net", self.player_net[0].net)
    ...
    def initialize(self, base_info, diff_data, game_setting):
        ...
        # self.num_to_alive = {"alive":0,"attacked":1,"execute":2}
    ...
    def selectAgent(self,target_role):
        ...
        est_werewolf_list = self.daily_net[use_model].net(np.asarray([self.base_info["day"],np.sum(self.co_list,axis=0)[1]] + self.daily_vector.reshape(-1).tolist()).astype(np.float32).reshape(1,-1)).reshape(-1).array
        est_werewolf_list = list(zip(est_werewolf_list,range(1,len(est_werewolf_list)+1)))
        est_role_list = []
        for i in range(self.agent_num):
            role = np.argmax(self.player_net[use_model].net(np.asarray([self.base_info["day"],np.sum(self.co_list,axis=0)[1]] + self.daily_vector[i,:].reshape(-1).tolist()).astype(np.float32).reshape(1,-1)).reshape(-1).array)
    ...
    def plot_accu_loss(self):
        ...
        for i in range(len(self.player_net)):
            if 0 < len(self.player_net[i].memory.accuracy_memory):
                player_loss.plot(self.player_net[i].memory.accuracy_memory,label="day"+str(i))
                plt.legend()
    ...
    def daily_model_eval(self):
        if self.each_model == True:
            ...
            daily_x = np.array(self.daily_x[1:]).astype(np.float32)
            daily_t = np.tile(self.daily_t,(daily_x.shape[0],1)).astype(np.int32)
        ...
        if self.each_model == True:
            ...
            for i in range(len(self.player_x)):
                if 0 < len(self.player_x[i]):
                    ...
                    self.update_predict_result(player_y, np.array(self.player_t).astype(np.int32))
                    self.player_net[0].memory.addLossAccuracy(loss,accuracy)
