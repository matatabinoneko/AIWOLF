...
class data_info():
    ...
    def selectAgent(self,target_role):
        ...
        est_werewolf_list = self.daily_net[use_model].net(np.asarray(common_vector + self.daily_vector.reshape(-1).tolist()).astype(np.float32).reshape(1,-1)).reshape(-1).array
        est_werewolf_list = list(zip(est_werewolf_list,range(1,len(est_werewolf_list)+1)))
        est_role_list = []
        for i in range(self.agent_num):
            role = np.argmax(self.player_net[use_model].net(np.asarray(common_vector + self.daily_vector[i,:].reshape(-1).tolist()).astype(np.float32).reshape(1,-1)).reshape(-1).array)
...
class MLP(chainer.Chain):
        ...
        def __call__(self, x):
            h1 = F.dropout(F.relu(self.l1(x)),ratio=0.6)
            h2 = F.dropout(F.relu(self.l2(h1)),ratio=0.6)
            h3 = self.l3(h2)
            return h3
